...
class Search:
    ...
    def forward(self, query:str='i want to store a json', *extra_query, **kwargs):
        query = ' '.join([query, *extra_query])
        module2schema = self.module2schema()
        options = []
        ...
        context  = f'''
        '''
        return self.select(query=query, options=options)
    ...
    def select(self,  
        ...
        if trials > 0 :
            try:
                return self.select(options=options, query=query, n=n, trials=trials-1, threshold=threshold, context=context, model=model)
        ...
        for ch in self.model.forward(prompt, model=model, stream=1): 
            print(ch, end='')
            output += ch
            if ch == anchors[1]:
                break
    ...
    def module2schema(self, max_age=100,  **kwargs):
        modules = c.modules()
        path = 'module2schema'
        module2schema = c.get(path, {}, max_age=max_age)
        if len(module2schema) > 0:
        ...
        for module in modules:
            try:
                ...
                module2schema[module] = schema
                # check if it is json serialziable
            except Exception as e:
                print(e)
                continue

        c.put(path, module2schema)
        return module2schema
