...
import { X, Shield, Key as KeyIcon, FileSignature, CheckCircle, XCircle, LogOut, ShieldCheck, ChevronDown } from 'lucide-react'
...
export const UserProfile = ({ user, isOpen, onClose, keyInstance, onLogout }: UserProfileProps) => {
  ...
  return (
    <div
      className={`fixed top-[60px] right-0 h-[calc(100vh-60px)] w-96 bg-black border-l border-green-500 transform transition-transform duration-300 z-40 ${
        isOpen ? 'translate-x-0' : 'translate-x-full'
      }`}
    >
      <div className="p-6 font-mono h-full flex flex-col">
        {/* Header with Key as Title and Logout Button */}
        <div className="flex items-center justify-between mb-6 border-b border-green-500/30 pb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={onLogout}
              className="flex items-center gap-1 px-3 py-1.5 bg-red-900/20 text-red-400 border border-red-500/50 hover:bg-red-500 hover:text-white hover:border-red-500 transition-all uppercase font-mono text-sm font-bold shadow-lg hover:shadow-red-500/30"
              title="Logout"
            >
              <LogOut size={16} />
              <span>LOGOUT</span>
