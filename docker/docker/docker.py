...
class Docker:
    ...
    def build(self,
        ...
        print(f'Building Docker image {tag} from {path}')
    ...
    def images(self, to_records: bool = False) -> Union[pd.DataFrame, Any]:
        """
        ...
        try:
            ...
            headers = [x.lower() for x in re.split(r'\s{2,}', lines[0].strip())]
            ...
            for line in lines[1:]:
                ...
                # Split by multiple spaces, but be careful with the TAG column
                parts = re.split(r'\s{2,}', line.strip())
                
                # Handle case where REPOSITORY and TAG might be combined
            ...
            df['image'] = df['image id']
            del df['image id']
            
            return df.to_dict('records') if to_records else df
        except Exception as e:
